...
GOOGLE_CLIENT_ID = "519196160578-fc322mcfc6ofc7e2eqiau5j7mp4csipp.apps.googleusercontent.com"
...
def sleep():
   if session.get("google_access_token"):
    ...
    return render_template('sleepdisorder.html', steps=steps, age=age, systolic=systolic, diastolic=diastolic, rate=rate, sleepdura=sleepdura)
   else:
...
def srisk():
    if request.method == 'POST':
            ...
            # print(input_data)
            # Reshape input data
            # input_data_array = np.asarray(input_data, dtype=np.int64)
            # input_data_reshaped = input_data_array.reshape(1, -1)
            # prediction = ensem_sleep.predict(input_data_reshaped)
